...
while game_start:
    ...
    # Because it's 'True' that introduction_choice is not assigend 1 and 2. Meaning neither valid choice was selected
...
# Also, I try to utlize trailing "else" statements, so I catch all possiblities of what the user could input.
...
# Certain organizations utilize julian dates, particularly in the goverment sector. So I figured I would try
        ...
        if log_computer_input == "3":
            ...
            if log_computer_input == "2":
                ...
                if log_computer_input == "1":
                    ...
                    # I have to format the interger value of 'current_julian_date' to be a string. In order to
...
#######################################################################################################################
# Importing time module for text/readability reasons
...
def monster_movement(location1, location2):
    ...
    room_repeat = True
    monster_next_location = "Not None"
    ...
    while player_left_room == False:
        ...
        if player_choice == "1":
            ...
            if monster_current_room == "solar":
                # A check to determine if the monster has moved into the same room as the player. The player would not be
                ...
                # Also, if the monster literally moves into the room that the player is in as he's moving away, they'll have
